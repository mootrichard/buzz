...
///    exist unless the caller explicitly allows an untagged event (used for DMs,
///    whose membership already defines the private audience). Tag kind is checked
///    via `tag.as_slice()` for stable, library-independent access.
...
pub async fn match_event(
    ...
    for (index, rule) in rules.iter().enumerate() {
        ...
        if rule.require_mention && !allow_untagged {
...
mod tests {
    ...
    #[tokio::test]
    async fn direct_message_rule_accepts_a_p_tag_for_a_stale_agent_identity() {
        let agent_pubkey = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef";
        let stale_agent_pubkey = "badc0ffee0ddf00dbadc0ffee0ddf00dbadc0ffee0ddf00dbadc0ffee0ddf00d";
        let event = make_event_with_p_tag(9, "@Frame hey homie", stale_agent_pubkey);
        let channel_id = any_channel();
        let rules = vec![make_rule(
            "mention-only",
            ChannelScope::All("all".into()),
        ...
        let matched = match_event(&event, channel_id, &rules, agent_pubkey, true).await;

        assert_eq!(
            matched.map(|value| value.prompt_tag),
            Some("@mention".into())
        );
    }
