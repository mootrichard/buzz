...
pub(crate) async fn publish_remote_deployment(
    ...
    if desired_state == DeploymentDesiredState::Running {
        ...
        let prompt = match crate::managed_agents::effective_config::resolve_effective_config(
            record, &personas, &global,
        ) {
            crate::managed_agents::effective_config::EffectiveConfigResult::Resolved(config) => {
...
async fn try_change_remote_agent_state(
    ...
    build_managed_agent_summary(app, updated, &runtimes, &personas, &global_config).map(Some)
}
